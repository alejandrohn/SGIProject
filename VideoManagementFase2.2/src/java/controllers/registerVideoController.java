...
public class registerVideoController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            rg.insertVideo(autor, title, description, format, createdDate, reproductionNumber, hour, minute, seconds, videoPath);
//            request.getRequestDispatcher("/WEB-INF/videoList.jsp").forward(request, response);
    ...
    public Boolean existUser(HttpSession session) {
     ...
     private void setUrlNotLogging(HttpServletRequest request) {
        String url = request.getRequestURL().append('?').append(request.getQueryString()).toString();
        request.setAttribute("oldUrl", url);
